...
function getNextFriday(date) {
  ...
  function addDays(days) {
    return new Date(curDate.setDate(monthday + days));
  }
...
function isDateInPeriod(date, period) {
  const min = new Date(period.start);
  const max = new Date(period.end);

  return min <= new Date(date) && new Date(date) <= max;
}
...
function getNextFridayThe13th(date) {
  function isFriday13th(dateToCheck) {
    return dateToCheck.getDay() === 5 && dateToCheck.getDate() === 13;
  }

  const currentDate = date;

  while (!isFriday13th(currentDate)) {
...
function getQuarter(date) {
  ...
  const quarterInMs = (1000 * 60 * 60 * 24 * 365) / 4;

  const quarter = Math.ceil(diff / quarterInMs);
